...
/* Universal Cyberpunk Neon theme overrides */
...
/* Universal Classic Dracula theme overrides */
body.theme-dracula {
...
/* Circular scroll progress HUD layout styling */
...
@media (max-width: 1024px) {
  /* Force 1-column layout on all templates to prevent content squeeze */
  .layout, .premium-layout {
    grid-template-columns: 1fr !important;
  }
  
  /* Reset body padding to prevent layout push */
  body {
    padding-left: 0 !important;
  }
  body.sidebar-open {
  ...
  /* Force sidebars to be a slide-out overlay drawer on mobile */
  .sidebar, .sidebar-hud {
    position: fixed !important;
    top: 0 !important;
    left: -300px !important; /* Hide off-screen by default */
    width: 300px !important;
    max-width: 85vw !important; /* Cap width for very small screens */
    height: 100vh !important;
    z-index: 1008 !important;
    background: rgba(11, 12, 16, 0.98) !important;
    backdrop-filter: blur(20px) !important;
    border-right: 1px solid var(--border, #2a2c38) !important;
    box-shadow: 10px 0 40px rgba(0, 0, 0, 0.6) !important;
    padding: 85px 20px 48px 20px !important;
    display: block !important; /* Override display: none */
    opacity: 1 !important;
    pointer-events: auto !important;
    transition: left 0.3s cubic-bezier(0.25, 1, 0.5, 1) !important;
  }
  body.light-theme .sidebar, body.light-theme .sidebar-hud {
    background: rgba(248, 250, 252, 0.98) !important;
    border-right-color: var(--border, #cbd5e1) !important;
    box-shadow: 10px 0 40px rgba(15, 23, 42, 0.15) !important;
  }
  
  /* Open state overrides */
  body.sidebar-open .sidebar,
  body.sidebar-open .sidebar-hud,
  .sidebar.open,
  .sidebar-hud.open {
    left: 0 !important;
  }
  
  /* Hide sidebar-closed width collapses that spoil layout */
  body.sidebar-closed .sidebar,
  body.sidebar-closed .sidebar-hud {
  ...
  /* Sleek Scrollable Pill Dock for Controls */
  .controller-rack-hud {
    position: fixed !important;
    top: 16px !important;
    right: 16px !important;
    left: auto !important;
    max-width: calc(100% - 76px) !important; /* Avoid colliding with toggle button */
    display: flex !important;
    gap: 6px !important;
    ...
    border: 1px solid var(--border, #2a2c38) !important;
    border-radius: 24px !important;
    z-index: 1009 !important;
    scrollbar-width: none !important;
    box-shadow: 0 8px 24px rgba(0, 0, 0, 0.3) !important;
  }
  .controller-rack-hud::-webkit-scrollbar {
    display: none !important;
  }
  body.light-theme .controller-rack-hud {
  ...
  body.light-theme .hud-btn:active, body.light-theme .hud-btn:hover {
    background: rgba(0, 0, 0, 0.05) !important;
  }

  /* Make sure main content padded comfortably */
  main {
    padding: 85px 16px 80px 16px !important;
  }
  .hero, .hero-radar {
    padding: 100px 16px 60px 16px !important;
    min-height: auto !important;
  }
  .hero-scroll, .hero-scroll-indicator {
    display: none !important;
  }

  /* Prevent grid overflow in lists and columns */
  .mistakes-grid, .test-cards, .three-things, .company-cards, .error-breakdown, .terms-grid,
  .pitfall-matrix, .retrieval-grid, .enterprise-radar, .summary-hud-grid, .glossary-matrix-grid {
    grid-template-columns: 1fr !important;
    gap: 16px !important;
  }
  
  .code-block, .premium-code-block {
  ...
  #scratch-pane-flashcards, #scratch-pane-quiz {
    flex: 1 !important;
...
/* ========================================================
   PREMIUM UX INTERACTION UPGRADES
   ======================================================== */

/* 1. Scrollspy Active Link Styles */
body.phase-0 { --phase-active-color: #6366f1; }
body.phase-1, body.phase-11, body.phase-13 { --phase-active-color: #06b6d4; }
body.phase-2, body.phase-10 { --phase-active-color: #8b5cf6; }
body.phase-3, body.phase-15 { --phase-active-color: #f43f5e; }
body.phase-4, body.phase-18 { --phase-active-color: #f59e0b; }
body.phase-5, body.phase-12, body.phase-17 { --phase-active-color: #10b981; }
body.phase-6 { --phase-active-color: #6366f1; }
body.phase-7, body.phase-16 { --phase-active-color: #f97316; }
body.phase-8, body.phase-14 { --phase-active-color: #2563eb; }
body.phase-9 { --phase-active-color: #ec4899; }

body.light-theme.phase-0 { --phase-active-color: #4338ca; }
body.light-theme.phase-1, body.light-theme.phase-11, body.light-theme.phase-13 { --phase-active-color: #0284c7; }
body.light-theme.phase-2, body.light-theme.phase-10 { --phase-active-color: #6d28d9; }
body.light-theme.phase-3, body.light-theme.phase-15 { --phase-active-color: #db2777; }
body.light-theme.phase-4, body.light-theme.phase-18 { --phase-active-color: #d97706; }
body.light-theme.phase-5, body.light-theme.phase-12, body.light-theme.phase-17 { --phase-active-color: #047857; }
body.light-theme.phase-6 { --phase-active-color: #4338ca; }
body.light-theme.phase-7, body.light-theme.phase-16 { --phase-active-color: #c2410c; }
body.light-theme.phase-8, body.light-theme.phase-14 { --phase-active-color: #1d4ed8; }
body.light-theme.phase-9 { --phase-active-color: #be185d; }

.nav-item.active, .hud-nav-item.active {
  color: var(--phase-active-color, var(--purple, #8b5cf6)) !important;
  font-weight: 800 !important;
  position: relative;
  text-shadow: 0 0 10px rgba(139, 92, 246, 0.15);
  background: rgba(139, 92, 246, 0.04) !important;
}
.nav-item.active::before, .hud-nav-item.active::before {
  content: "";
  position: absolute;
  left: 0;
  top: 10%;
  height: 80%;
  width: 3px;
  background: var(--phase-active-color, var(--purple, #8b5cf6)) !important;
  border-radius: 0 4px 4px 0;
  box-shadow: 0 0 8px var(--phase-active-color);
}

/* 2. AI Highlight Explainer Tooltip */
.floating-ai-tooltip {
  position: absolute;
  z-index: 9999;
  background: rgba(19, 20, 37, 0.95);
  backdrop-filter: blur(12px);
...
.floating-ai-tooltip:hover {
  background: var(--p2-grad, linear-gradient(135deg, #8b5cf6 0%, #d946ef 100%));
  border-color: transparent;
  transform: translateY(-2px);
  box-shadow: 0 12px 40px rgba(139, 92, 246, 0.5);
  transition: all 0.2s ease;
}
body.light-theme .floating-ai-tooltip {
  background: rgba(255, 255, 255, 0.95);
  border-color: rgba(124, 58, 237, 0.3);
  color: #0f172a;
  box-shadow: 0 8px 32px rgba(124, 58, 237, 0.15);
}
body.light-theme .floating-ai-tooltip:hover {
  background: var(--p2-grad, linear-gradient(135deg, #7c3aed 0%, #a21caf 100%));
  color: #ffffff;
}
...
body.light-theme .sidebar-progress-widget {
  border-top-color: var(--border, rgba(0, 0, 0, 0.06));
...
body.light-theme .progress-ring-text {
  color: var(--text, #0f172a);
}
body.light-theme .progress-ring-track {
...
/* 4. Immersive HUD Audio Player Redesign */
#audio-control-panel {
  top: auto !important;
  bottom: 85px !important;
  right: 24px !important;
  background: rgba(19, 20, 26, 0.85) !important;
  border: 1px solid rgba(139, 92, 246, 0.3) !important;
  border-radius: var(--radius-md, 16px) !important;
  padding: 18px !important;
  box-shadow: 0 20px 50px rgba(0, 0, 0, 0.6), 0 0 20px rgba(139, 92, 246, 0.15) !important;
...
body.light-theme #audio-control-panel {
  ...
  box-shadow: 0 20px 50px rgba(15, 23, 42, 0.08), 0 0 20px rgba(124, 58, 237, 0.08) !important;
}
body.light-theme #audio-control-panel.playing {
  box-shadow: 0 20px 50px rgba(15, 23, 42, 0.08), 0 0 30px rgba(124, 58, 237, 0.2) !important;
...
.audio-equalizer.animating .eq-bar {
  animation: eq-bounce 0.8s ease-in-out infinite alternate !important;
}
.audio-equalizer:not(.animating) .eq-bar {
...
/* 5. Mark Completed HUD State overrides */
#hud-complete-btn.completed:hover {
  background: rgba(16, 185, 129, 0.15) !important;
  border-color: #10b981 !important;
  color: #10b981 !important;
  box-shadow: 0 0 15px rgba(16, 185, 129, 0.35) !important;
}
#hud-complete-btn.completed span {
  color: #10b981 !important;
}
...
body.light-theme #ai-tutor-suggestions {
  background: rgba(0, 0, 0, 0.02);
  border-top-color: rgba(0, 0, 0, 0.05);
}
.ai-tutor-suggestion-btn {
  background: rgba(139, 92, 246, 0.08);
...
.ai-tutor-suggestion-btn span {
  font-size: 0.8rem;
}
.ai-tutor-suggestion-btn.chip-summary {
  background: rgba(139, 92, 246, 0.08);
  border-color: rgba(139, 92, 246, 0.25);
  color: #c084fc;
}
.ai-tutor-suggestion-btn.chip-summary:hover {
  background: rgba(139, 92, 246, 0.18);
  border-color: rgba(139, 92, 246, 0.5);
  color: #e9d5ff;
}
body.light-theme .ai-tutor-suggestion-btn.chip-summary {
  background: rgba(124, 58, 237, 0.05);
  border-color: rgba(124, 58, 237, 0.2);
  color: #6d28d9;
}
body.light-theme .ai-tutor-suggestion-btn.chip-summary:hover {
  background: rgba(124, 58, 237, 0.12);
  border-color: rgba(124, 58, 237, 0.4);
  color: #4c1d95;
}
.ai-tutor-suggestion-btn.chip-quiz {
  background: rgba(46, 196, 182, 0.08);
  border-color: rgba(46, 196, 182, 0.25);
  color: #2ec4b6;
}
.ai-tutor-suggestion-btn.chip-quiz:hover {
  background: rgba(46, 196, 182, 0.18);
  border-color: rgba(46, 196, 182, 0.5);
  color: #a7f3d0;
}
body.light-theme .ai-tutor-suggestion-btn.chip-quiz {
  background: rgba(13, 148, 136, 0.05);
  border-color: rgba(13, 148, 136, 0.2);
  color: #0f766e;
}
body.light-theme .ai-tutor-suggestion-btn.chip-quiz:hover {
  background: rgba(13, 148, 136, 0.12);
  border-color: rgba(13, 148, 136, 0.4);
  color: #115e59;
}
.ai-tutor-suggestion-btn.chip-takeaways {
  background: rgba(245, 158, 11, 0.08);
  border-color: rgba(245, 158, 11, 0.25);
  color: #f59e0b;
}
.ai-tutor-suggestion-btn.chip-takeaways:hover {
  background: rgba(245, 158, 11, 0.18);
  border-color: rgba(245, 158, 11, 0.5);
  color: #fde68a;
}
body.light-theme .ai-tutor-suggestion-btn.chip-takeaways {
  background: rgba(217, 119, 6, 0.05);
  border-color: rgba(217, 119, 6, 0.2);
  color: #b45309;
}
body.light-theme .ai-tutor-suggestion-btn.chip-takeaways:hover {
...
#ai-voice-input-btn:hover {
  background: rgba(255, 255, 255, 0.1) !important;
  color: var(--text-hyper-bright, #f8fafc) !important;
}
#ai-voice-input-btn.listening {
  background: rgba(239, 68, 68, 0.15) !important;
...
body.light-theme #ai-voice-input-btn {
  background: rgba(0,0,0,0.03) !important;
  border-color: rgba(0,0,0,0.08) !important;
  color: #64748b !important;
}
body.light-theme #ai-voice-input-btn:hover {
  background: rgba(0,0,0,0.08) !important;
  color: #0f172a !important;
}
body.light-theme #ai-voice-input-btn.listening {
...
(function() {
  let savedTheme = localStorage.getItem('faang-course-theme-preset');
  if (!savedTheme) {
    const legacy = localStorage.getItem('faang-course-theme');
    savedTheme = legacy === 'light' ? 'light' : 'dark';
  }
  const body = document.body;
  body.classList.remove('light-theme', 'theme-cyberpunk', 'theme-dracula');
  if (savedTheme === 'light') body.classList.add('light-theme');
  else if (savedTheme === 'cyberpunk') body.classList.add('theme-cyberpunk');
  else if (savedTheme === 'dracula') body.classList.add('theme-dracula');
})();

function toggleThemeHUD() {
  const themes = ['dark', 'cyberpunk', 'dracula', 'light'];
  let savedTheme = localStorage.getItem('faang-course-theme-preset');
  if (!savedTheme) {
    const legacy = localStorage.getItem('faang-course-theme');
    savedTheme = legacy === 'light' ? 'light' : 'dark';
  ...
  else if (nextTheme === 'cyberpunk') body.classList.add('theme-cyberpunk');
  else if (nextTheme === 'dracula') body.classList.add('theme-dracula');
  
  localStorage.setItem('faang-course-theme-preset', nextTheme);
  localStorage.setItem('faang-course-theme', nextTheme === 'light' ? 'light' : 'dark');
  
  if (themeText) {
    let displayName = "Dark Cyber";
    if (nextTheme === 'cyberpunk') displayName = "Cyber Neon";
    if (nextTheme === 'dracula') displayName = "Dracula";
    if (nextTheme === 'light') displayName = "Light Lab";
    themeText.innerText = displayName;
  }
  syncScratchpadTheme(nextTheme === 'light' ? 'light' : 'dark');
}
...
function initSidebarState() {
  ...
  const actualState = savedState || (isMobile ? 'closed' : 'open');
...
function getCurrentPhase() {
  const body = document.body;
  for (let p = 1; p <= 18; p++) {
    if (body.classList.contains('phase-' + p)) return 'phase-' + p;
  }
  return 'phase-1';
}
...
const flashcardData = {
  ...
  "phase-5": [
    { q: "What is Next.js Partial Prerendering (PPR)?", a: "It combines static shells with dynamic streaming content in the same route page, serving the static structure instantly while streaming slow data." },
    { q: "How does the Next.js Data Cache persist?", a: "It caches fetch request results across server requests and deployments, bypassing the network unless revalidated or opt-out flags are passed." },
    { q: "What is Next.js Middleware execution order?", a: "Runs before the request hits the route handler, allowing authentication redirects, header modifications, or custom logging checks." }
  ],
  "phase-6": [
    { q: "What is Chrome DevTools Performance Profiling?", a: "It records runtime performance timelines, identifying layout shifts, long tasks (>50ms), garbage collection spikes, and CPU bottlenecks." },
    { q: "What are VS Code snippets used for?", a: "Reusable code templates triggered by keyboard shortcuts, reducing repetitive code boilerplate typing for common structures." },
    { q: "What are Postman Environment Variables?", a: "Dynamic keys that swap values (like URLs or Auth tokens) depending on whether you're targeting Local, Staging, or Production environments." }
  ],
  "phase-7": [
    { q: "Explain Node.js non-blocking I/O event loop.", a: "It offloads file, network, or process operations to the system kernel (via libuv). When done, a callback triggers back in the single main thread." },
    { q: "What is Express routing middleware order?", a: "Middleware executes in the order it is declared using app.use(). If next() is not called, the request is left hanging or terminated." },
    { q: "What is Zod parsing schema approach?", a: "Zod parses inputs against a structural schema, returning clean typed outputs or throwing verbose error messages for validation failures." }
  ],
  "phase-8": [
    { q: "What are ACID database transactions?", a: "Atomicity (all or nothing), Consistency (valid state transitions), Isolation (independent transactions), and Durability (persistence)." },
    { q: "How does a database B-Tree index work?", a: "Creates a balanced tree traversal structure to look up rows in log(n) time, bypassing slow full-table scans for matches." },
    { q: "What is Mongoose schema vs MongoDB collections?", a: "MongoDB is schema-less by default. Mongoose sits on top of Node.js to enforce document validation, schemas, and relational hooks." }
  ],
  "phase-9": [
    { q: "Why do we add salts to hashed passwords?", a: "A unique random string appended to the password before hashing, protecting against precomputed table attacks (Rainbow Tables)." },
    { q: "How does JWT authentication work?", a: "Tokens are signed by the server using a secret, containing client details. The client sends it in headers, and the server validates it without DB checks." },
    { q: "What is CORS (Cross-Origin Resource Sharing)?", a: "A browser security model checking headers (like Access-Control-Allow-Origin) to decide if a script can access APIs hosted elsewhere." }
  ],
  "phase-10": [
    { q: "What is Socket.io WebSocket connection upgrade?", a: "It starts with standard HTTP long polling for compatibility, and upgrades to a persistent, bidirectional WebSocket connection if supported." },
    { q: "What is Multer cloud storage pipeline?", a: "A multipart/form-data parser that streams file uploads to local memory buffers or disk before piping them up to Cloudinary/AWS S3." },
    { q: "How does Redis Session Caching speed up backends?", a: "Stores user sessions in memory rather than disk databases, enabling sub-millisecond retrieval of login states." }
  ],
  "phase-11": [
    { q: "Explain the CAP Theorem trade-offs.", a: "A distributed system can only guarantee two out of three: Consistency (same data), Availability (always answers), and Partition Tolerance (network splits)." },
    { q: "What is database sharding?", a: "Horizontal partitioning of tables, storing different rows on distinct server instances to scale reads, writes, and memory limits." },
    { q: "How do message queues (like Kafka) ensure durability?", a: "They append messages to write-ahead logs on disk, persisting events even if consuming microservices crash or disconnect." }
  ],
  "phase-12": [
    { q: "What is an LLM context window limit?", a: "The maximum number of tokens (words/characters) the model can parse and process at once for prompts and output answers." },
    { q: "What is Vector Database RAG?", a: "Retrieval-Augmented Generation. It queries external document embeddings, retrieves matches, and appends them to LLM prompts for factual grounding." },
    { q: "How does temperature affect LLM outputs?", a: "Ranges from 0 to 2. Low temperature (e.g. 0) yields deterministic and factual answers. High temperature yields creative and random variations." }
  ],
  "phase-13": [
    { q: "Explain the TCP 3-Way Handshake.", a: "1. Client sends SYN. 2. Server replies SYN-ACK. 3. Client sends ACK. This establishes a reliable, duplex connection channel." },
    { q: "What is browser layout and paint path?", a: "HTML parses to DOM + CSS parses to CSSOM -> Render Tree -> Layout (dimensions/coordinates) -> Paint (pixels) -> Composite Layers." },
    { q: "What is the difference between OS Process and Thread?", a: "A Process is an isolated running program with its own memory. A Thread is a lightweight segment of execution sharing its parent process memory." }
  ],
  "phase-14": [
    { q: "What is the key difference between Interface and Type in TS?", a: "Interfaces are extendable via declaration merging (repeating the same name merges keys). Types are static and merged via intersections (&)." },
    { q: "What does TS keyof operator do?", a: "Extracts all public property keys of an object type and returns them as a union type of string literal values." },
    { q: "What is TypeScript strictNullChecks?", a: "Enforces that null and undefined must be handled explicitly, avoiding common 'cannot read property of null' runtime errors." }
  ],
  "phase-15": [
    { q: "What is mock testing isolation?", a: "Replacing external dependencies (like databases or network APIs) with dummy mock modules to isolate the code unit and run tests offline." },
    { q: "Why are RTL (React Testing Library) user events preferred?", a: "They simulate real browser click and keyboard dispatch pathways, unlike fireEvent which only fires synthetic event wrappers." },
    { q: "What are Snapshot Tests useful for?", a: "Comparing current component HTML markup against a stored reference copy, alerting developers to unintentional UI structure changes." }
  ],
  "phase-16": [
    { q: "What is a Docker image vs container?", a: "A Docker image is a read-only template containing the application code and environment setup. A container is a running instance of that image." },
    { q: "What are GitHub Actions workflows runners?", a: "Virtual machines (Linux, Windows, macOS) hosted by GitHub that execute the CI/CD scripts when triggered by events like git push." },
    { q: "How do Sentry sourcemaps help debugging?", a: "They map minified production bundles back to the original source code lines, letting you see the exact file and line where error stacktraces crashed." }
  ],
  "phase-17": [
    { q: "What is database migration syncing?", a: "Programmatic scripts that safely evolve database schemas (tables/columns) across environments without wiping existing production tables data." },
    { q: "How do you scale WebSockets horizontally?", a: "Use a Pub/Sub redis adapter to broadcast event messages across distinct server instances, updating clients regardless of which machine they are connected to." },
    { q: "What is vector embedding semantic search?", a: "Converting queries to coordinate arrays and using cosine similarity to find documents with matching meanings, bypassing simple keyword match limits." }
  ],
  "phase-18": [
    { q: "What is the STAR interviewing framework?", a: "Situation (setting context), Task (identifying challenges), Action (what you coded/built), and Result (concrete metrics, outputs, or learning outcomes)." },
    { q: "How do you structure a System Design interview?", a: "1. Scope requirements. 2. Draw high-level blocks. 3. Estimate scale/loads. 4. Identify bottlenecks. 5. Detail component scaling." },
    { q: "What is a GitHub profile pin priority?", a: "Highlighting 2-3 production-grade capstone projects with clear readme guides, architectural diagrams, and links rather than pinning dozens of simple class exercises." }
  ]
};

const quizData = {
  "phase-1": [
    { q: "Which HTML element is semantically correct for main site navigation?", o: ["&lt;nav&gt;", "&lt;div class='nav'&gt;", "&lt;ul&gt;", "&lt;section&gt;"], a: 0 },
    { q: "Which Flexbox property aligns items along the cross-axis?", o: ["justify-content", "align-items", "flex-direction", "align-self"], a: 1 }
  ],
  "phase-2": [
    { q: "Which CSS contain value isolates layout, style, and paint?", o: ["contain: strict", "contain: content", "contain: paint", "contain: layout"], a: 0 },
    { q: "How are Container Queries specified in CSS?", o: ["@media (min-width: 400px)", "@container (min-width: 400px)", "@parent (min-width: 400px)", "@element (min-width: 400px)"], a: 1 }
  ],
  "phase-3": [
    { q: "Which queue are Promise callbacks appended to?", o: ["Macrotask Queue", "Render Queue", "Microtask Queue", "RequestAnimationFrame Queue"], a: 2 },
    { q: "What is the GC strategy used by modern V8 engines?", o: ["Reference Counting", "Mark-and-Sweep (Generational)", "Manual Deallocation", "Stop-and-Copy Only"], a: 1 }
  ],
  "phase-4": [
    { q: "What is the main role of standard key props in React lists?", o: ["To apply styling", "To uniquely identify siblings for diff optimization", "To store element states", "To bind click events"], a: 1 },
    { q: "Which hook handles side effects in React function components?", o: ["useState", "useContext", "useEffect", "useMemo"], a: 2 }
  ],
  "phase-5": [
    { q: "Where does Middleware run in Next.js routing lifecycle?", o: ["After page renders", "Before the request hits route handler/page", "Inside getServerSideProps", "Inside the browser main thread"], a: 1 },
    { q: "Which fetch tag revalidates Next.js data after a time limit?", o: ["{ cache: 'no-store' }", "{ next: { revalidate: 3600 } }", "{ force: 'dynamic' }", "{ mode: 'cors' }"], a: 1 }
  ],
  "phase-6": [
    { q: "Which DevTools panel profiles page load speed and CPU paint blocks?", o: ["Elements", "Application", "Performance", "Security"], a: 2 },
    { q: "What is Postman Collections used for?", o: ["Hosting static files", "Grouping related API requests for automated tests runs", "Running database migrations", "Writing CSS preprocessor configs"], a: 1 }
  ],
  "phase-7": [
    { q: "What is the callback signature of an Express error-handling middleware?", o: ["(req, res, next)", "(err, req, res, next)", "(req, res, err)", "(err, req, res)"], a: 1 },
    { q: "Which Zod method checks inputs and returns detailed status objects rather than throwing?", o: ["parse()", "safeParse()", "check()", "validate()"], a: 1 }
  ],
  "phase-8": [
    { q: "Which SQL command aggregates rows based on matching column groups?", o: ["WHERE", "ORDER BY", "GROUP BY", "SELECT DISTINCT"], a: 2 },
    { q: "What database index speeds up rows matches lookup in O(log n)?", o: ["Clustered Index only", "Hash index", "B-Tree Index", "Bitmask Index"], a: 2 }
  ],
  "phase-9": [
    { q: "What JWT component contains the signature validations keys?", o: ["Header", "Payload", "Crypto Signature Segment", "Metadata"], a: 2 },
    { q: "Which node module protects Express APIs by adding standard secure HTTP headers?", o: ["cors", "helmet", "dotenv", "bcrypt"], a: 1 }
  ],
  "phase-10": [
    { q: "Which HTTP header is sent by browsers to request WebSocket upgrades?", o: ["Connection: Keep-Alive", "Upgrade: websocket", "Content-Type: text/event-stream", "Transfer-Encoding: chunked"], a: 1 },
    { q: "Which Redis command publishes message events to channels?", o: ["SET", "PUBLISH", "SUBSCRIBE", "LPUSH"], a: 1 }
  ],
  "phase-11": [
    { q: "In CAP theorem, what does Partition Tolerance mean?", o: ["Data remains consistent", "The system functions during network split errors", "The system responds instantly to requests", "Database sharding is enabled"], a: 1 },
    { q: "Which database MVCC mechanism solves concurrency issues?", o: ["Explicit Row Locking", "Multi-Version Concurrency Control", "Single-threaded writes queue", "Global Database Mutex"], a: 1 }
  ],
  "phase-12": [
    { q: "What is the embedding vector extraction phase in RAG?", o: ["Model output sampling", "Document search lookup", "Converting text data into high-dimensional coordinate arrays", "Prompt templating validation"], a: 2 },
    { q: "Which LLM temperature setting yields highly factual responses?", o: ["temperature: 1.8", "temperature: 0.0", "temperature: 1.0", "temperature: 2.0"], a: 1 }
  ],
  "phase-13": [
    { q: "What does the SYN-ACK handshake segment signify in TCP?", o: ["Connection termination", "Client acknowledging data", "Server acknowledging client SYN and requesting client ACK", "DNS lookup completion"], a: 2 },
    { q: "What is the worst-case Big O time complexity of Binary Search?", o: ["O(1)", "O(n)", "O(log n)", "O(n log n)"], a: 2 }
  ],
  "phase-14": [
    { q: "Which TS utility extracts public keys of an object structure?", o: ["keyof", "typeof", "readonly", "any"], a: 0 },
    { q: "What is the main advantage of Types over Interfaces in TypeScript?", o: ["Declaration merging", "Allowing union types and intersection aliases", "Faster compilation speed", "Direct runtime checking support"], a: 1 }
  ],
  "phase-15": [
    { q: "Which RTL query priority level is recommended for accessible elements checks?", o: ["getByTestId", "getByText", "getByRole", "querySelector"], a: 2 },
    { q: "What testing method mocks external server API fetches calls?", o: ["Unit Testing isolation via mocks", "E2E testing live calls", "Snapshot regressions", "Static typing scans"], a: 0 }
  ],
  "phase-16": [
    { q: "Which Dockerfile instruction specifies the default command when a container starts?", o: ["RUN", "COPY", "CMD", "FROM"], a: 2 },
    { q: "What does GitHub Actions workflow runner execute?", o: ["Browser renders", "The YAML CI workflow tasks", "Vercel deployments only", "Sentry tracking hooks"], a: 1 }
  ],
  "phase-17": [
    { q: "Which database mechanism resolves conflicts during concurrent writes in live systems?", o: ["Optimistic Locking via version columns", "Full database locks", "Offline batch processing", "Manual admin review"], a: 0 },
    { q: "What Redis adapter scales real-time chat horizontal relays?", o: ["Redis Pub/Sub Adapter", "Redis JSON module", "Redis Cluster hash", "Redis Graph database"], a: 0 }
  ],
  "phase-18": [
    { q: "What STAR method section describes the specific technical actions you took?", o: ["Situation", "Task", "Action", "Result"], a: 2 },
    { q: "What is the first step of the System Design Interview template?", o: ["Write SQL schemas", "Scope requirements and scale limits", "Design deep database caches", "Configure network load balancers"], a: 1 }
  ]
};
...
function renderFlashcard() {
  ...
  const lessonId = window.currentLessonId || '1.1';
  let deck = (window.customAIFlashcards && window.customAIFlashcards[lessonId]) 
              ? window.customAIFlashcards[lessonId] 
              : (flashcardData[phase] || flashcardData['phase-1']);
  ...
  const isCustom = !!(window.customAIFlashcards && window.customAIFlashcards[lessonId]);
  ...
  pane.innerHTML = `
    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%; font-size: 0.7rem; color: var(--text-muted, #64748b); margin-bottom: 4px;">
      ...
      <span style="text-transform: uppercase; font-weight: bold; color: ${isCustom ? '#8b5cf6' : 'var(--teal, #0d9488)'}; font-family: 'Space Mono', monospace;">
        ${isCustom ? '✨ AI Generated' : phase.replace('-', ' ')}
      </span>
    </div>
    ...
    <div class="flashcard-container" onclick="this.classList.toggle('flipped')">
      <div class="flashcard-inner">
        <div class="flashcard-front">
          <div style="font-size: 0.78rem; font-weight: 500; line-height: 1.4;">${deck[currentFlashcardIndex].q}</div>
          <div class="flashcard-prompt">&#128260; Tap to Flip</div>
        </div>
        <div class="flashcard-back">
          <div style="font-size: 0.75rem; font-weight: 400; line-height: 1.4;">${deck[currentFlashcardIndex].a}</div>
    ...
    <div style="width: 100%; margin-top: 8px; display: flex; gap: 6px;">
      <button id="ai-generate-flashcards-btn" onclick="generateAIFlashcards()" style="background: linear-gradient(135deg, #8b5cf6 0%, #d946ef 100%); border: none; border-radius: 4px; padding: 8px 12px; color: #ffffff; font-size: 0.7rem; font-weight: 700; cursor: pointer; flex: 1; text-align: center; box-shadow: 0 4px 10px rgba(139, 92, 246, 0.2); transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 4px;">
        <span>✨ AI Generate Cards</span>
      </button>
      ${isCustom ? `
      <button onclick="resetDefaultFlashcards()" style="background: transparent; border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; padding: 6px 8px; color: #64748b; font-size: 0.65rem; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.color='#f8fafc'" onmouseout="this.style.color='#64748b'" title="Reset to default deck">
        &#8634;
      </button>` : ''}
    </div>
  `;
...
function prevFlashcard(e) {
  ...
  const phase = getCurrentPhase();
  const lessonId = window.currentLessonId || '1.1';
  const deck = (window.customAIFlashcards && window.customAIFlashcards[lessonId]) 
              ? window.customAIFlashcards[lessonId] 
              : (flashcardData[phase] || flashcardData['phase-1']);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Combined Reading Progress & Scroll to Top Tracker
  const progressHud = document.getElementById('reading-progress-hud');
  ...
  if (progressHud) {
    const updateProgress = () => {
      ...
      if (progressCircle) {
        const perimeter = 2 * Math.PI * 18; // 113.1
      ...
      if (scrollTop > 350) {
        ...
        progressHud.style.opacity = '1';
        progressHud.style.pointerEvents = 'auto';
      } else {
        progressHud.style.opacity = '0';
        progressHud.style.pointerEvents = 'none';
      }
    };
  ...
  if (themeText) {
    let displayName = "Dark Cyber";
    if (savedTheme === 'cyberpunk') displayName = "Cyber Neon";
    if (savedTheme === 'dracula') displayName = "Dracula";
    if (savedTheme === 'light') displayName = "Light Lab";
    themeText.innerText = displayName;
  }
  syncScratchpadTheme(savedTheme === 'light' ? 'light' : 'dark');
  
  initClickToPlay();
  initHighlightExplainer();
  initSidebarProgressWidget();
  initScrollspy();
  updateHUDCompleteBtn();
  ...
  if (window.visualViewport) {
    const handleViewportResize = () => {
      ...
      const scratchpad = document.getElementById('scratchpad-window');
      
      if (!isMobile) {
        if (tutorDrawer) tutorDrawer.style.height = '';
        if (scratchpad) scratchpad.style.height = '';
        return;
      }
      
      const vh = window.visualViewport.height;
      
      // Update AI Tutor height dynamically to fit visible area above keyboard
      if (tutorDrawer && (tutorDrawer.style.display === 'flex' || tutorDrawer.classList.contains('open'))) {
        tutorDrawer.style.height = `${vh}px`;
      }
      
      // Update Scratchpad height dynamically
      if (scratchpad && window.getComputedStyle(scratchpad).display === 'flex') {
        scratchpad.style.height = `${vh}px`;
      }
    };
    ...
    // Bind trigger to toggle actions
    const originalToggleAITutor = window.toggleAITutorDrawer || toggleAITutorDrawer;
    window.toggleAITutorDrawer = () => {
      originalToggleAITutor();
      setTimeout(handleViewportResize, 50);
      setTimeout(handleViewportResize, 150);
    };
    
    const originalToggleScratchpad = window.toggleScratchpadHUD || toggleScratchpadHUD;
    window.toggleScratchpadHUD = () => {
      originalToggleScratchpad();
      setTimeout(handleViewportResize, 50);
      setTimeout(handleViewportResize, 150);
    };
  }
...
window.addEventListener('storage', (e) => {
  ...
  if (e.key === 'faang-course-theme-preset') {
    const t = e.newValue || 'dark';
    const body = document.body;
    const themeText = document.getElementById('theme-text');
    body.classList.remove('light-theme', 'theme-cyberpunk', 'theme-dracula');
    if (t === 'light') body.classList.add('light-theme');
    else if (t === 'cyberpunk') body.classList.add('theme-cyberpunk');
    else if (t === 'dracula') body.classList.add('theme-dracula');
    if (themeText) {
      let displayName = "Dark Cyber";
      if (t === 'cyberpunk') displayName = "Cyber Neon";
      if (t === 'dracula') displayName = "Dracula";
      if (t === 'light') displayName = "Light Lab";
      themeText.innerText = displayName;
    }
    syncScratchpadTheme(t === 'light' ? 'light' : 'dark');
  }
});
...
function submitTutorSuggestion(promptText) {
  const input = document.getElementById('ai-chat-input');
  if (!input) return;
  input.value = promptText;
  sendAITutorMessage();
}
window.submitTutorSuggestion = submitTutorSuggestion;

// --- PREMIUM FEATURE: HIGHLIGHT & ASK AI EXPLAINER ---
function initHighlightExplainer() {
  let tooltip = document.getElementById('floating-ai-tooltip');
  if (!tooltip) {
  ...
  const handleSelection = () => {
    ...
    const isInsideMain = container.nodeType === Node.ELEMENT_NODE 
      ? container.closest('main') 
      : container.parentElement?.closest('main');
...
// --- PREMIUM FEATURE: SIDEBAR PROGRESS COMPLETION RING ---
function initSidebarProgressWidget() {
  const sidebar = document.querySelector('.sidebar, .sidebar-hud');
  if (!sidebar) return;
  if (document.getElementById('sidebar-progress-widget')) return;
  
  const widget = document.createElement('div');
  widget.id = 'sidebar-progress-widget';
  widget.className = 'sidebar-progress-widget';
  widget.innerHTML = `
    <div class="progress-ring-container">
      <svg class="progress-ring-svg" width="48" height="48">
        <circle class="progress-ring-track" stroke="rgba(255,255,255,0.05)" stroke-width="4" fill="transparent" r="20" cx="24" cy="24"/>
        <circle class="progress-ring-circle" stroke="url(#sidebar-progress-grad)" stroke-width="4" fill="transparent" r="20" cx="24" cy="24" stroke-dasharray="125.66 125.66" stroke-dashoffset="125.66"/>
        <defs>
      ...
      <div class="progress-ring-text" id="sidebar-progress-percent">0%</div>
    </div>
    <div class="progress-widget-meta">
      <div class="progress-widget-label">Phase Progress</div>
      <div class="progress-widget-sub" id="sidebar-progress-count">0 of 0 Completed</div>
    </div>
  `;
...
function updateSidebarProgress() {
  ...
  const completed = JSON.parse(localStorage.getItem('faang-completed-essays') || '[]');
  const lessonsInPhase = window.phaseLessonIds || [];
  
  if (lessonsInPhase.length === 0) {
    circle.style.strokeDashoffset = circumference;
    percentText.innerText = '0%';
    if (countText) countText.innerText = '0 of 0 Completed';
    return;
  }
  
  const completedInPhase = lessonsInPhase.filter(id => completed.includes(id)).length;
  ...
  if (countText) {
    countText.innerText = `${completedInPhase} of ${totalInPhase} Completed`;
  }
}

// --- PREMIUM FEATURE: MARK ESSAY COMPLETION ---
function toggleCurrentEssayCompletion() {
  if (!window.currentLessonId) return;
  let completed = JSON.parse(localStorage.getItem('faang-completed-essays') || '[]');
  const index = completed.indexOf(window.currentLessonId);
  
  if (index === -1) {
    completed.push(window.currentLessonId);
    
    const today = new Date().toDateString();
    const lastActive = localStorage.getItem('faang-last-active-day');
    let streak = parseInt(localStorage.getItem('faang-learning-streak') || '0', 10);
    if (lastActive !== today) {
      if (lastActive) {
      ...
      localStorage.setItem('faang-learning-streak', streak);
      localStorage.setItem('faang-last-active-day', today);
    }
  } else {
    completed.splice(index, 1);
  }
  
  localStorage.setItem('faang-completed-essays', JSON.stringify(completed));
...
function updateHUDCompleteBtn() {
  const btn = document.getElementById('hud-complete-btn');
  if (!btn) return;
  const completed = JSON.parse(localStorage.getItem('faang-completed-essays') || '[]');
  const isCompleted = completed.includes(window.currentLessonId);
  const textSpan = document.getElementById('hud-complete-text');
  
  if (isCompleted) {
    btn.classList.add('completed');
    if (textSpan) textSpan.innerText = 'Completed';
    btn.style.borderColor = '#10b981';
    btn.style.color = '#10b981';
    btn.style.boxShadow = '0 0 10px rgba(16, 185, 129, 0.2)';
  } else {
    btn.classList.remove('completed');
    if (textSpan) textSpan.innerText = 'Mark Complete';
...
// --- PREMIUM FEATURE: SCROLLSPY INTERACTION ---
...
// Storage event synchronization for multi-tab support
window.addEventListener('storage', (e) => {
  if (e.key === 'faang-completed-essays') {
    updateHUDCompleteBtn();
    updateSidebarProgress();
  }
});

window.initHighlightExplainer = initHighlightExplainer;
window.initSidebarProgressWidget = initSidebarProgressWidget;
window.updateSidebarProgress = updateSidebarProgress;
window.toggleCurrentEssayCompletion = toggleCurrentEssayCompletion;
window.updateHUDCompleteBtn = updateHUDCompleteBtn;
window.initScrollspy = initScrollspy;

// Speech-to-Text Voice Dictation Feature
let voiceRecognition = null;
function toggleVoiceInput() {
  const btn = document.getElementById('ai-voice-input-btn');
  ...
  if (btn.classList.contains('listening')) {
    ...
    voiceRecognition.onresult = (event) => {
      ...
      if (resultText) {
        ...
        chatInput.value = val ? val + " " + resultText : resultText;
...
// AI Flashcards Feature (Dynamic Session Cache)
if (!window.customAIFlashcards) {
  window.customAIFlashcards = {};
}

async function generateAIFlashcards() {
  const btn = document.getElementById('ai-generate-flashcards-btn');
  if (btn) {
    btn.disabled = true;
  ...
  loader.id = 'flashcard-ai-loader';
  loader.style.cssText = 'position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(17, 20, 37, 0.95); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; border-radius: 12px; z-index: 10; padding: 20px; text-align: center; color: #ffffff;';
  loader.innerHTML = `
    <div style="width: 24px; height: 24px; border: 2.5px solid rgba(139, 92, 246, 0.2); border-top-color: #8b5cf6; border-radius: 50%; animation: spin 1s linear infinite;"></div>
    <div style="font-size: 0.75rem; font-weight: 500; font-family: 'Plus Jakarta Sans', sans-serif;">AI is scanning this essay and generating custom revision cards...</div>
  `;
  
  const scratchpadWindow = document.getElementById('scratchpad-window');
  if (scratchpadWindow) {
    scratchpadWindow.style.position = 'relative';
    scratchpadWindow.appendChild(loader);
  }

  const essayText = getEssayTextForContext();
  const systemPrompt = `You are a FAANG interview examiner.
Your task is to analyze the syllabus text below and generate 5 highly effective revision flashcards.
Each flashcard must contain a short question ("q") and a concise answer ("a") (under 25 words).
Respond ONLY with a valid JSON array of exactly 5 objects, where each object has "q" and "a" properties. Do not include markdown formatting or backticks outside the JSON.
...
---`;
  ...
  try {
    ...
    const data = await response.json();
    let responseText = key ? data.candidates[0].content.parts[0].text : data.reply;
    ...
    const lessonId = window.currentLessonId || '1.1';
    window.customAIFlashcards[lessonId] = cards;
    currentFlashcardIndex = 0;
    renderFlashcard();
  } catch (error) {
    console.error("AI Flashcard Generation error:", error);
    alert("Could not generate AI flashcards: " + error.message);
  } finally {
    if (loader) loader.remove();
    if (btn) {
      btn.disabled = false;
      btn.innerText = "✨ AI Generate Cards";
      btn.style.opacity = '1';
    }
  }
}

function resetDefaultFlashcards() {
  const lessonId = window.currentLessonId || '1.1';
  if (window.customAIFlashcards) {
    delete window.customAIFlashcards[lessonId];
  }
  currentFlashcardIndex = 0;
  renderFlashcard();
}

window.generateAIFlashcards = generateAIFlashcards;
window.resetDefaultFlashcards = resetDefaultFlashcards;
...
<div class="controller-rack-hud">
  ...
  <button class="hud-btn" id="hud-complete-btn" onclick="toggleCurrentEssayCompletion()" aria-label="Mark Essay Completed">
    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 6px;"><polyline points="20 6 9 17 4 12"></polyline></svg>
    <span id="hud-complete-text">Mark Complete</span>
  </button>
  <button class="hud-btn" id="auth-hud-btn" onclick="toggleAuthModal()" aria-label="Account Login">
    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 6px;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
    <span>Log In</span>
  </button>
...
<div id="scratchpad-hud-container" style="position: fixed; bottom: 30px; right: 30px; z-index: 1010; font-family: 'Plus Jakarta Sans', sans-serif;">
  <!-- Circular Reading Progress HUD (Back to Top) -->
  <div id="reading-progress-hud" onclick="scrollToTopHUD()" aria-label="Scroll to Top" style="display: none; opacity: 0; pointer-events: none; width: 50px; height: 50px; border-radius: 50%; background: var(--surface, #13141a); border: 2px solid var(--border, #2a2c38); color: var(--text, #e4e2dc); align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3); z-index: 1008; transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1); margin-bottom: 12px; outline: none; position: relative;">
    <svg width="44" height="44" viewBox="0 0 44 44" style="transform: rotate(-90deg); width: 44px; height: 44px; display: block;">
      <circle cx="22" cy="22" r="18" fill="none" stroke="var(--border, #2a2c38)" stroke-width="2.5" />
      <circle id="reading-progress-circle" cx="22" cy="22" r="18" fill="none" stroke="var(--accent, #38bdf8)" stroke-width="2.5" stroke-dasharray="113.1" stroke-dashoffset="113.1" stroke-linecap="round" style="transition: stroke-dashoffset 0.1s ease;" />
    </svg>
    <span id="reading-progress-text" style="position: absolute; font-family: 'JetBrains Mono', monospace; font-size: 0.65rem; font-weight: 700; color: var(--text, #e4e2dc);">0%</span>
  </div>
  <!-- Toggle Button -->
...
<div id="ai-tutor-drawer" style="display: none; position: fixed; top: 0; right: -400px; width: 400px; height: 100vh; background: var(--bg-surface-dark, #0a0c14); border-left: 2px solid var(--border-neon-vivid, #cbd5e1); z-index: 2000; box-shadow: -10px 0 40px rgba(0,0,0,0.5); transition: right 0.4s cubic-bezier(0.16, 1, 0.3, 1); flex-direction: column; font-family: 'Plus Jakarta Sans', sans-serif;">
  ...
  <!-- Suggestions Row -->
  <div id="ai-tutor-suggestions">
    <button class="ai-tutor-suggestion-btn chip-summary" onclick="submitTutorSuggestion('Summarize this module')">Summarize Module</button>
    <button class="ai-tutor-suggestion-btn chip-quiz" onclick="submitTutorSuggestion('Give me 3 quick quiz questions')">Quiz Me</button>
    <button class="ai-tutor-suggestion-btn chip-takeaways" onclick="submitTutorSuggestion('List key takeaways')">Key Takeaways</button>
  </div>

  <!-- Input Form -->
  <div id="ai-tutor-input-container" style="padding: 12px 16px 36px 16px; border-top: 1px solid rgba(255,255,255,0.06); display: flex; gap: 8px; background: rgba(0,0,0,0.15); align-items: flex-end;">
    <textarea id="ai-chat-input" placeholder="Ask a question about this roadmap module..." rows="3" style="flex: 1; background: #070913; border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; padding: 10px 12px; color: #ffffff; font-size: 0.8rem; resize: none; outline: none; line-height: 1.4; font-family: inherit; height: 80px;" onkeydown="handleAITutorKeyDown(event)"></textarea>
    <button id="ai-voice-input-btn" onclick="toggleVoiceInput()" style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; color: #64748b; cursor: pointer; transition: all 0.2s; margin-bottom: 22px;" title="Voice input (Speech to Text)">
      <svg class="mic-icon" viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"></path><path d="M19 10v2a7 7 0 0 1-14 0v-2"></path><line x1="12" y1="19" x2="12" y2="23"></line><line x1="8" y1="23" x2="16" y2="23"></line></svg>
    </button>
    <button onclick="sendAITutorMessage()" style="background: linear-gradient(135deg, #8b5cf6 0%, #d946ef 100%); border: none; border-radius: 6px; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; color: #ffffff; cursor: pointer; box-shadow: 0 4px 10px rgba(139, 92, 246, 0.3); margin-bottom: 22px;">
...
foreach ($file in $htmlFiles) {
    ...
    if ($file.Name -match "essay-(\d+)\.(\d+)\.html") {
        ...
        # Build array of all lessons in this phase
        $phaseLessonIds = @()
        foreach ($f in $htmlFiles) {
            if ($f.Name -match "essay-$currentPhase\.(\d+)\.html") {
                $phaseLessonIds += "'$currentPhase.$($Matches[1])'"
            }
        }
        $jsLessonsArray = "[" + ($phaseLessonIds -join ", ") + "]"
