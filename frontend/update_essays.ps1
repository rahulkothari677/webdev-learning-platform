...
function initThreeBackground() {
  ...
  try {
    ...
    // Dashboard specific theme color variables (fallback just in case)
    ...
    // 1. CORNER PERSPECTIVE SQUARE GRIDS (Straight lines, square cells)
    // Left Grid: Cyan/Color1 accent, positioned on the left side
    const gridSize = 100;
    const gridDivisions = 25;
    
    const leftGrid = new THREE.GridHelper(gridSize, gridDivisions, colorLeft, colorLeft);
    leftGrid.rotateX(Math.PI / 2); // Make it vertical
    leftGrid.position.set(-42, 0, -25);
    leftGrid.rotation.y = Math.PI / 4.5; // Angled from corner towards camera
    
    leftGrid.material.transparent = true;
    leftGrid.material.opacity = isLightInitial ? 0.02 : 0.08; // Ultra subtle grid
    leftGrid.material.blending = THREE.AdditiveBlending;
    leftGrid.material.depthWrite = false;
    scene.add(leftGrid);

    // Right Grid: Magenta/Color2 accent, positioned on the right side
    const rightGrid = new THREE.GridHelper(gridSize, gridDivisions, colorRight, colorRight);
    rightGrid.rotateX(Math.PI / 2); // Make it vertical
    rightGrid.position.set(42, 0, -25);
    rightGrid.rotation.y = -Math.PI / 4.5; // Angled from corner towards camera
    
    rightGrid.material.transparent = true;
    rightGrid.material.opacity = isLightInitial ? 0.02 : 0.08; // Ultra subtle grid
    rightGrid.material.blending = THREE.AdditiveBlending;
    rightGrid.material.depthWrite = false;
    scene.add(rightGrid);

    // Set visibility based on initial screen width (hidden on mobile/tablet)
    const isMobileInitial = window.innerWidth < 1024;
    leftGrid.visible = !isMobileInitial;
    rightGrid.visible = !isMobileInitial;
    ...
    for (let i = 0; i < bgDotCount; i++) {
      bgPositions[i * 3] = (Math.random() - 0.5) * 65;
      bgPositions[i * 3 + 1] = (Math.random() - 0.5) * 45;
      bgPositions[i * 3 + 2] = (Math.random() - 0.5) * 30 - 15;
    ...
    const bgDotMat = new THREE.PointsMaterial({
      ...
      vertexColors: true,
      transparent: true,
      opacity: isLightInitial ? 0.12 : 0.35,
      map: neonTexture,
    ...
    for (let i = 0; i < fgDotCount; i++) {
      fgPositions[i * 3] = (Math.random() - 0.5) * 60;
      fgPositions[i * 3 + 1] = (Math.random() - 0.5) * 40;
    ...
    const fgDotMat = new THREE.PointsMaterial({
      size: 2.4, // Clean, glowing floating point size
      vertexColors: true,
      transparent: true,
      opacity: isLightInitial ? 0.22 : 0.75,
      map: neonTexture,
      blending: THREE.AdditiveBlending,
      depthWrite: false
    });
    ...
    const animate = () => {
      ...
      // Gentle, slow swaying on Y/Z axis (no spinning/circular motion)
      leftGrid.rotation.y = Math.PI / 4.5 + Math.sin(time * 0.08) * 0.03;
      leftGrid.rotation.z = Math.cos(time * 0.06) * 0.015;

      rightGrid.rotation.y = -Math.PI / 4.5 - Math.sin(time * 0.08) * 0.03;
      rightGrid.rotation.z = -Math.cos(time * 0.06) * 0.015;
      ...
      for (let i = 0; i < bgDotCount; i++) {
        ...
        if (Math.abs(bgPosArr[i * 3]) > 32) bgDrift[i].x *= -1;
        if (Math.abs(bgPosArr[i * 3 + 1]) > 22) bgDrift[i].y *= -1;
      }
      ...
      for (let i = 0; i < fgDotCount; i++) {
        ...
        if (Math.abs(fgPosArr[i * 3]) > 30) fgDrift[i].x *= -1;
        if (Math.abs(fgPosArr[i * 3 + 1]) > 20) fgDrift[i].y *= -1;
      }
      fgDotGeom.attributes.position.needsUpdate = true;

      const isLight = document.body.classList.contains('light-theme');
      leftGrid.material.opacity = isLight ? 0.02 : 0.08;
      rightGrid.material.opacity = isLight ? 0.02 : 0.08;
      
      const baseOpacity = isLight ? 0.22 : 0.55;
      const pulseRange = isLight ? 0.08 : 0.2;
      fgDotMat.opacity = baseOpacity + Math.sin(time * 1.2) * pulseRange;
    ...
    window.addEventListener('resize', () => {
      ...
      const isMobile = window.innerWidth < 1024;
      leftGrid.visible = !isMobile;
      rightGrid.visible = !isMobile;
    });
